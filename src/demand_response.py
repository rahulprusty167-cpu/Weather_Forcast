...
class DemandResponseEngine:
    ...
    All recipient logs, phone numbers, and customer profiles generated by this engine
    are completely synthetic mock data for demonstration purposes only.
    Real user phone numbers entered in frontend forms are never stored, logged, or
    persisted to any database, file, or session history.
    """

    def __init__(self):
        # Stores only synthetically generated demonstration dispatch records
    ...
    def execute_automated_dr_dispatch(
        ...
        for i in range(min(12, pool_size)):
            ...
            sms_text = (
                f"[DEMO] Grid Relief Program: Critical transformer stress in {area} ({pincode}). "
                f"Reduce ~{expected_reduction_kw} kW for {duration_hours} hrs to earn ₹{micro_discount_rate_inr}/kWh "
                f"(est. ₹{rebate_earned} bill credit - simulated, not a real transaction). Reply YES to confirm."
            )
